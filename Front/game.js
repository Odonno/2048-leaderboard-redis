...
function loadLeaderboard() {
    fetch(baseApiUrl + '/leaderboard/scores')
        .then(response => {
            return response.json();
        })
        .then(data => {
            var scores = data.sort(function (a, b) { return b.points - a.points; });

            function getLeaderboardRowText(score, rank) {
                if (score) {
                    return ('#' + rank + ' ' + score.username + ' - ' + score.points + ' points')
                }
            ...
            for (var index = 0; index < 5; index++) {
                var rank = index + 1;
                document.getElementById('ranking-' + rank).innerText = getLeaderboardRowText(scores[index], rank);
            }
        });
}

function saveScore(payload) {
    fetch(baseApiUrl + '/leaderboard/save', {
        method: 'POST',
...
var gameObj = {
    ...
    gameOver: function () {
        if (this.gameOverApplied) {
            return;
        }

        var username = prompt("GAME OVER! Set your username to save the score:");

        if (username) {
            var payload = {
                username,
                points: this.points.score
            };

            saveScore(payload);
        }

        this.gameOverApplied = !this.gameOverApplied;
    },
...
window.onload = function () {
    ...
    function keyUp(e) {
        ...
        switch (currKey) {
            ...
            case 32: gameObj.fastPlay(); break;
        }
    }
